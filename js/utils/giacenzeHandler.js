...
export async function handleGiacenzaEffettiva(settore, codice) {
    try {
        // Create or get modal
        let modal = document.getElementById('giacenzeModal');
        if (!modal) {
            modal = createGiacenzeModal();
        }
        ...
        // Get giacenza data
        const { data: giacenze, error: giacenzaError } = await supabase.rpc('get_giacenza_effettiva', {
            _settore: settore,
            _articolo_cod: codice
        });

        if (giacenzaError) throw giacenzaError;
        if (!giacenze) throw new Error('Nessun dato disponibile');

        const [giacenza] = giacenze;
        ...
        // Show error in modal
        const modal = document.getElementById('giacenzeModal');
        if (modal) {
            const contentContainer = modal.querySelector('.modal-content');
            contentContainer.innerHTML = `
                <span class="modal-close">&times;</span>
...
function createGiacenzeModal() {
    const modalHTML = `
        <div id="giacenzeModal" class="modal">
